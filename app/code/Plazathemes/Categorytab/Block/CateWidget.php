...
class CateWidget extends \Magento\Catalog\Block\Product\AbstractProduct implements \Magento\Widget\Block\BlockInterface
{
	...
	public function getProductCate2($id = NULL) {
        ...
        //load the category's products as a collection
		 $_productCollection = $this->productCollectionFactory->create()
               ->addAttributeToSelect('*')
               ->addCategoryFilter($_category)
			   // ->addAttributeToFilter('special_price', array('eq' => null))
			   ;
			 $qty = $this->getConfig('qty');	
			 if($qty<1) $qty = 8;
			 $_productCollection ->setPageSize($qty);
	...
	public function getSaleproduct($id = NULL)
	{
	...
	public function getcategoriesbyid($value=''){
		$qty = 8;
        $collection = $this->_categoryFactory->create()
			...
			->addFieldToFilter('parent_id', array('eq'=>$value))
			->addFieldToFilter('is_active', array('eq'=>'1'))
			->setPageSize($qty);
        return $collection;
	}
	
	function getCatetabImage($image=null) {
